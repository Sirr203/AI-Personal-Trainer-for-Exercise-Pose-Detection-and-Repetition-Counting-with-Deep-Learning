...
class LungePoseAnalysis:
    """
    Phân tích tư thế Lunge sử dụng 2 model và logic dựa trên luật để tăng độ ổn định.
    - Sửa lỗi model nhận nhầm trạng thái "down" thành "init" bằng cách kiểm tra góc gối.
    """
    def __init__(self, stage_model, error_model, scaler, 
        ...
        # Ngưỡng góc để xác định một chân có đang gập không (dùng để sửa lỗi model)
        self.BENT_KNEE_THRESHOLD = 160 
    ...
    def _get_knee_angles(self, results):
        """Helper: Tính và trả về góc của cả hai đầu gối."""
        landmarks = results.pose_landmarks.landmark
    ...
    def analyze_pose(self, results):
        ...
        # 1. Dự đoán giai đoạn (init, mid, down)
        ...
        # 2. <<< CẢI TIẾN: LOGIC SỬA LỖI CHO MODEL >>>
        # Nếu model dự đoán là "init" với độ tin cậy cao, nhưng thực tế cả 2 gối đều đang gập,
        # thì khả năng cao đây là trạng thái "down" bị nhận nhầm.
        if stage_predicted_class == "I" and stage_confidence >= self.prob_threshold:
            left_knee_angle, right_knee_angle = self._get_knee_angles(results)
            # Nếu cả hai gối đều gập (góc nhỏ hơn ngưỡng), ta ghi đè dự đoán của model
            if left_knee_angle < self.BENT_KNEE_THRESHOLD and right_knee_angle < self.BENT_KNEE_THRESHOLD:
                stage_predicted_class = "D"  # Ghi đè thành "down"
                # print("LOG: Model predicted 'init', but overridden to 'down' based on knee angles.") # Dòng debug (tùy chọn)

        # 3. Cập nhật trạng thái và bộ đếm
        if stage_confidence >= self.prob_threshold:
            if stage_predicted_class == "I":
                ...
                # Chỉ tăng bộ đếm khi chuyển từ "init" hoặc "mid" sang "down"
                if self.current_stage in ["mid", "init"]:
                    self.counter += 1
                self.current_stage = "down"

        # 4. Kiểm tra lỗi CHỈ khi ở giai đoạn "down"
    ...
    def _check_knee_angles(self, results):
        left_angle, right_angle = self._get_knee_angles(results)
...
def main():
    ...
    out_writer = cv2.VideoWriter(OUTPUT_VIDEO_PATH, fourcc, input_fps, (output_width, output_height))
    print(f"Đang lưu video output vào '{OUTPUT_VIDEO_PATH}' với kích thước {output_width}x{output_height}, FPS={input_fps}")
        ...
        with torch.no_grad():  # Thêm context manager cho YOLO
            while cap.isOpened():
                ...
                if results.pose_landmarks:
                    ...
                    analysis_result = detector.analyze_exercise(results, 
                    ...
                    cv2.putText(image, f"DETECTED: {current_detected_exercise.upper()} ({detection_confidence:.2f})", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
